...
- **other_clauses** 是 **SELECT** 语句支持的子句。 **SELECT** 语句支持很多子句，包括：
  - 使用 **[DISTINCT](./030.DISTINCT%20去重.md#概述)** 运算符选择不同的行。
  - 使用 **[ORDER BY](./010.ORDER%20BY%20排序.md#概述)** 子句对行进行排序。
  - 使用 **[WHERE](./005.WHERE%20条件.md#概述)** 子句过滤行。
  - 使用 **[LIMIT](./020.LIMIT%20限制返回的行数.md#概述) or [FETCH](./015.FETCH%20限制返回的行数.md#概述)** 子句从表中选择行的子集。
  - 使用 **[GROUP BY](./075.GROUP%20BY%20分组.md#概述)** 子句将行分组。
  - 使用 [HAVING](./080.HAVING%20分组的过滤条件.md#概述) 子句过滤组。
  - 使用诸如 **INNER JOIN, LEFT JOIN, FULL OUTER JOIN, CROSS JOIN** 之类的与其他表 [连接](./100.JOIN%20链接.md#概述)。
  - 使用 **[UNION](./115.UNION%20并集.md#概述)** ， **[INTERSECT](./120.INTERSECT%20交集.md#概述)** 和 **[EXCEPT](./125.EXCEPT%20差集.md#概述)** 执行集合运算。

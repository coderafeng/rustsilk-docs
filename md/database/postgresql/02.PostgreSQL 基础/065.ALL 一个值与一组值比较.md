...
- **array** 是一个数组或者**[子查询](./105.子查询.md#概述)**，它跟在 **ALL** 运算符后面。子查询必须用括号括起来。
...
您可以在 **[WHERE](./005.WHERE%20条件.md#概述)** 子句中按照如下方式使用 **ALL** 运算符：
...
PostgreSQL **ALL** 运算符用于将一个值与一个**子查询**返回的所有的值进行比较，并返回一个布尔值。 **ALL** 运算符可用于 **WHERE** 子句中过滤指定条件行。

...
在 PostgreSQL 中，您可以在 **[SELECT](./000.SELECT.md#概述)** 语句中使用 **LIMIT** 子句来限定返回的行的数量。

注意， **LIMIT** 子句虽然被很多数据库系统支持，但它不是 SQL 标准。 为了符合 SQL 标准，您可以是用 **[FETCH](./015.FETCH%20限制返回的行数.md#概述)** 子句来完成同样的事情。
...
其中，**other_clauses** 是那些可以在 **SELECT** 语句中使用的其他子句，比如 **[WHERE](./005.WHERE%20条件.md#概述)**, **[ORDER BY](./010.ORDER%20BY%20排序.md#概述)**, **[OFFSET](./025.OFFSET%20查询跳过指定行数.md#概述)** 等
...
要限制从 **[film](../01.PostgreSQL%20起步/04.PostgreSQL%20示例数据库.md#影片表)** 表查询时最多返回 5 行，请使用下面的带有 **LIMIT** 子句的 **SELECT** 语句：

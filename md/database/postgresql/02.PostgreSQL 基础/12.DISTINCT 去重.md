...
在 PostgreSQL 中，DISTINCT 语句用于 SELECT 语句中，以使其返回一个没有重复行的结果集。

# DISTINCT 语法

要返回一个没有重复行的结果集，请按如下语法使用带有 DISTINCT 的 SELECT语句：
...
始终将 ORDER BY 子句与 the DISTINCT ON(expression) 一起使用以使结果集可预测是一种很好的做法。
...
使用 UNION ALL 模拟包含多个 NULL 值的记录集。

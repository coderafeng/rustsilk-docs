...
- **aggregate_function(ci)** 是聚合函数，用来汇总。这是可选的。 您可以会用到以下聚合函数：
  - **[SUM()](../07.PostgreSQL%20内置函数/060.聚合函数.md#sum)**: 计算组内数据的总和
  - **[AVG()](../07.PostgreSQL%20内置函数/060.聚合函数.md#avg)**: 计算组内数据的平均值
  - **[MAX()](../07.PostgreSQL%20内置函数/060.聚合函数.md#max)**: 计算组内数据的最大值
  - **[MIN()](../07.PostgreSQL%20内置函数/060.聚合函数.md#min)**: 计算组内数据的最小值
  - **[COUNT()](../07.PostgreSQL%20内置函数/060.聚合函数.md#count)**: 计算组内的行数
- **[SELECT](./000.SELECT.md#概述)** 后的字段必须是分组字段中的字段。
- **[WHERE](./005.WHERE%20条件.md#概述)** 子句是可选的，用来过在分组之前过滤行。
- **[HAVING](./080.HAVING%20分组的过滤条件.md#概述)** 子句是可选的，用来过滤分组数据。

# GROUP BY 示例

## 简单示例

使用 **GROUP BY** 子句查看 **[actor](../01.PostgreSQL%20起步/04.PostgreSQL%20示例数据库.md#演员表)** 表中的姓氏列表。
...
本例的输出结果与以下使用 **[DISTINCT](./030.DISTINCT%20去重.md#概述)** 的 SQL 输出结果完全一样：
...
3. 最后使用 **[ORDER BY](./010.ORDER%20BY%20排序.md#概述)** 子句按照 **count(*)** 降序排列。这样，数量最多的姓氏排在最前面。

同样，如果我们想从 **[film](../01.PostgreSQL%20起步/04.PostgreSQL%20示例数据库.md#影片表)** 表中查找每个影片等级的影片数量，请使用如下语句：
...
以下实例使用 **GROUP BY** 子句，**[LIMIT](./020.LIMIT%20限制返回的行数.md#概述)** 子句和聚合函数 **sum()** 从 **[payment](../01.PostgreSQL%20起步/04.PostgreSQL%20示例数据库.md#支付表)** 表中查找消费金额排名前 10 位的客户。
...
2. 然后使用聚合函数 **sum(amount)** 对每组中的所有行的 **amount** 字段求和，并使用 **total** 作为**[列别名](./035.AS%20别名.md#概述)**。

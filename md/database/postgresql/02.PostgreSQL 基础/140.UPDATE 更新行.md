...
- 使用 **[WHERE](./005.WHERE%20条件.md#概述)** 子句指定要更新的行的条件。只有符合 **WHERE** 条件的行才会被更新。
...
在以下实例中，**[customer](../01.PostgreSQL%20起步/04.PostgreSQL%20示例数据库.md#客户表)** 表中存储了客户的信息。

## 修改单列值

把 **customer_id** 等于 **1** 的客户的电子邮件修改为 **NEW.MARY.SMITH@sakilacustomer.org**。

1. 使用以下 **[SELECT](./000.SELECT.md#概述)** 语句查看更新前的数据。
```sql

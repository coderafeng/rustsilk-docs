...
相比较于 **[INSERT](./130.INSERT%20插入新行.md#概述)** 语句来说，**INSERT ON CONFLICT** 只是多了 **ON CONFLICT** 子句。

- **conflict_target** 是存在冲突的对象，它可以是以下之一：
  - 一个列名。该列必须是主键或者唯一索引。
  - **ON CONSTRAINT constraint_name**， 并且 **constraint_name** 必须是**[唯一约束](../03.PostgreSQL%20数据库和表/075.唯一约束.md#概述)** 的名称。
  - **[WHERE](./005.WHERE%20条件.md#概述)** 子句。
...
为了演示，我们需要**[创建一个新表](../03.PostgreSQL%20数据库和表/020.创建表.md#概述)**，命名为 **users**：

...
```mermaid
venn-beta
  title "A 表与 B 表的内连接（交集）"
  accTitle: A 表和 B 表内连接维恩图
  accDescr: A 表与 B 表的交集区域表示 INNER JOIN 的结果
  "A 表" : 20
  "B 表" : 30
  "A 表" & "B 表" : 50
```
...
```mermaid
venn-beta
  title "A 表 LEFT JOIN B 表（结果 = A 表全部）"
  accTitle: A 表和 B 表左连接维恩图
  accDescr: 左连接返回 A 表的全部记录，包括仅在 A 表中的记录以及 A 与 B 的交集记录
  "A 表" : 50
  "B 表" : 30
  "A 表" & "B 表" : 20
```
...
```mermaid
venn-beta
  title "A 表 RIGHT JOIN B 表（结果 = B 表全部）"
  accTitle: A 表和 B 表右连接维恩图
  accDescr: 右连接返回 B 表的全部记录，包括仅在 B 表中的记录以及 A 与 B 的交集记录
  "A 表" : 50
  "B 表" : 30
  "A 表" & "B 表" : 20
```
...
```mermaid
venn-beta
  title "A 表 FULL OUTER JOIN B 表（结果 = A 表 ∪ B 表）"
  accTitle: A 表和 B 表全连接维恩图
  accDescr: 全连接返回 A 表和 B 表的全部记录，即两个集合的并集，包括仅在 A 表、仅在 B 表以及两者交集的所有记录
  "A 表" : 50
  "B 表" : 30
  "A 表" & "B 表" : 20
```
